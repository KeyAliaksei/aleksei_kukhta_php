...
<head>
    <meta charset="UTF-8">
    <title>files_date_time</title>
</head>
<body>
<?php
/*
    $file = fopen("file.txt", "a+") or die("Error!");
...
$words = ['прокуратор', 'легат'];
if($file = fopen("file.txt", "r")){
    while (!feof($file)){
        $text = fgets($file);
        $splitSentences = explode(".&nbsp;", $text);
        foreach ($splitSentences as $value){
            for($i = 0; $i <= count($words); $i++) {
                $x = stripos($value, $words($i));
                if ($x !== false) {
                    fwrite($result, $value . "\n");
                    } else {
