...
// Выделить цветом все вхождения какого-либо слова (подобрать из контекста) с любым регистром символов, не изменяя регистр оригинала.
$word = ['прокуратор', 'Прокуратор'];
$replaceWords = ['<span>прокуратор</span>', '<span>Прокуратор</span>'];
$string = addSpan($word, $replaceWords, $string);

// Выделить первую букву каждого предложения жирным шрифтом.
$firstLetter = preg_replace_callback('/(^|[.]\s*)([А-Я])/u', function($matches){
    return $matches[1]."<b>".$matches[2]."</b>";
}, $string);
$string = nl2br($firstLetter). "<br>";
echo $string;
...
$countWords = substr_count($string, ' ');
...
echo "<br>" .$html = pageCounter($page, $countPages);
?>
</body>
</html>